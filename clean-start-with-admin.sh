...
# =====================================================
# Nettoyage Complet et Redémarrage avec Admin
# =====================================================
...
echo ""
echo -e "${BLUE}11. Création de l'utilisateur admin...${NC}"
docker exec -i rirepair-postgres psql -U rirepair_user -d rirepair < create-admin-user.sql

echo ""
echo -e "${BLUE}12. Vérification des créneaux horaires...${NC}"
SLOT_COUNT=$(docker exec rirepair-postgres psql -U rirepair_user -d rirepair -t -c "SELECT COUNT(*) FROM schedule_slots;" 2>/dev/null || echo "0")
echo -e "${GREEN}✓ $SLOT_COUNT créneaux horaires créés${NC}"

echo ""
echo -e "${BLUE}13. Démarrage du frontend...${NC}"
docker run -d \
...
echo ""
echo -e "${BLUE}14. Attente du frontend (30 secondes)...${NC}"
sleep 30
